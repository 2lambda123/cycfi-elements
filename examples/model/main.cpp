...
application. You have the flexibility to structure your application according to
your preferences. Here's a breakdown of the essential steps:
...
through callbacks such as the button's `on_click` and the input text box's
`on_enter` hooks. These callbacks are invoked when the user clicks the button or
presses the enter key in the text box, for example.

   3. Ensure the GUI is updated to accurately represent the application state
whenever changes occur. The application may need the ability to inspect the
state of the GUI elements or set them to a specific state at any given time.

   A typical approach for steps 2 and 3 involves holding different GUI elements
as private members within an application or GUI class. These elements are
managed as shared pointers, created using the `share(e)` function and held in
the element hieiarchy using the `hold(p)` function. The GUI class oversees the
interconnections and presents a higher-level view to the application through a
well-defined API.

   This example presents a more elegant way to structure an elements application
using Models. The Model (elements/model.hpp) serves as an abstraction for a data
type that is linked to one or more user interface elements. The actual data is
accessed and modified through the `get` and `set` member functions of the
derived class. A user interface element can be linked to a `model` by supplying
an `update_function` via the `on_update(f)` member function.

   The Model does not care about the GUI element types it is interacting with.
It is an abstract data type that models its underlying data type (e.g. float,
int, enum, etc.). It looks and acts like a concrete type.
...
value to it:
   ...
   The `value_model<float>` is a derived class of the template class `model`. It
specifically addresses the typical scenario where the data is internally stored
by value within the class. The class includes `get` and `set` member functions
following the `Model` concept required by the model template, facilitating the
retrieval and modification of data.

      Note: While this example utilizes the `value_model<float>`, you have the
flexibility to create your custom subclass of the template class model to
implement more tailored methods of accessing your data.
...
from the application, but not necessarily, e.g. if it holds the data itself.
   5. The GUI and the application are both unaware of each other.

   Number 5 is an important design principle known as "decoupling," emphasizing
the separation of concerns and promoting independence between the GUI and the
application components. The advantages of decoupling in a software design
context include, modularity, scalability, and reusability, and ease of
maintainance.
...
different nodes within the elements hierarchy. This illustrates the approach of
designing the user interface based on models.
=================================================================================================*/
...
auto make_dial(my_model &model, view &view_) {
   // Make a dial
   auto dial_ptr = share(dial(radial_marks<20>(basic_knob<80>()), 1.0));

   // When the user interacts with the dial, this will be called.
   // We assign the new value to the model.
   dial_ptr->on_change = [&model](double val) {
      if (model._value != val) {
         model._value = val;
   ...
   model._value.on_update([&view_, dial_ptr](double val) {
      dial_ptr->value(val);
      view_.refresh(*dial_ptr);
   });

   auto control = radial_labels<15>(hold(dial_ptr),
...
auto make_preset_menu(my_model &model, view &view_) {
   ...
   auto preset_menu = selection_menu(
   ...
   return align_center(
       hsize(180, htile(label("Preset:"), hspace(10), preset_menu.first)));
}
...
auto make_input_box(my_model &model, view &view_) {
   ...
   model._value.on_update([&view_, input = tbox.second](double val) {
   ...
   tbox.second->on_enter = [&model, &view_](std::string_view text) {
      ...
      if (error != "") {
         auto &&on_ok = [&model]() {
            // When errors are enountered, reset the model's value to its
            // previous state.
            model._value.update();
         };
...
auto make_content(my_model &model, view &view_) {
   ...
   return margin({20, 20, 20, 20},
                 group(margin({20, 20, 20, 20},
                              vgrid(grid_coords, make_preset_menu(model, view_),
...
int main(int argc, char *argv[]) {
   ...
   view_.content(make_content(model, view_), background);
