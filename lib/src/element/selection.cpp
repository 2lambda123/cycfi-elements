...
void select(composite_base const &c, selectable *e, std::size_t index,
            int &_select_start, int &_select_end) {
   select_none(c);
   e->select(true);
   _select_start = _select_end = index;
}

bool select(composite_base const &c, composite_base::hit_info const &hit,
            int &_select_start, int &_select_end) {
...
bool action_select(composite_base const &c, composite_base::hit_info const &hit,
                   int &_select_start, int &_select_end, bool _multi_select) {
   if (auto e = find_element<selectable *>(hit.element_ptr)) {
      if (!_multi_select) {
         if (auto pe = find_element<selectable *>(&c.at(_select_start));
             pe && e != pe)
            pe->select(false);
      }
...
void shift_select(composite_base const &c, std::size_t index, int _select_start,
                  int &_select_end) {
...
bool shift_select(composite_base const &c, composite_base::hit_info const &hit,
                  int _select_start, int &_select_end) {
   if (auto e = find_element<selectable *>(hit.element_ptr)) {
...
void select_first(composite_base const &c, int &_select_start,
                  int &_select_end) {
   if (c.size()) {
...
void select_last(composite_base const &c, int &_select_start,
                 int &_select_end) {
   if (c.size()) {
...
void select_next(composite_base const &c, int &_select_start, int &_select_end,
                 bool shift) {
...
void select_prev(composite_base const &c, int &_select_start, int &_select_end,
                 bool shift) {
   if ((_select_end - 1) >= 0) {
      if (shift)
         shift_select(c, _select_end - 1, _select_start, _select_end);
      ...
      in_context_do(ctx, *c, [&](context const &cctx) {
         ...
         if (hit.element_ptr) {
            if (btn.modifiers & mod_action) {
               ...
               if (btn.down)
                  r = action_select(*c, hit, _select_start, _select_end,
                                    _multi_select);
   ...
   if (k.action == key_action::press || k.action == key_action::repeat) {
      ...
      case key_code::up: {
         if (auto c = find_subject<composite_base *>(this)) {
            ...
            if (_select_end != -1) {
               in_context_do(ctx, *c, [&](context const &cctx) {
                  scrollable::find(ctx).scroll_into_view(
                      c->bounds_of(cctx, _select_end));
                  ctx.view.refresh(ctx.bounds);
               });
               r = true;
            }
...
int selection_list_element::get_select_start() const { return _select_start; }

int selection_list_element::get_select_end() const { return _select_end; }
