...
bool drop_base::wants_control() const { return true; }
...
void drop_base::track_drop(context const &ctx, drop_info const &info,
...
drop_inserter_base::drop_inserter_base(
    std::initializer_list<std::string> mime_types_)
    : base_type{mime_types_} {}
...
void drop_inserter_base::track_drop(context const &ctx, drop_info const &info,
                                    cursor_tracking status) {
   base_type::track_drop(ctx, info, status);
   if (is_tracking()) {
      static constexpr auto offset = 20;
      rect r = {info.where.x - offset, info.where.y - offset,
                info.where.x + offset, info.where.y + offset};
...
void draggable_element::draw(context const &ctx) {
   ...
   if (is_enabled()) {
      ...
      auto r = override_theme(&theme::label_font_color,
...
public:
   drag_image_element(std::size_t num_boxes) : _num_boxes{num_boxes} {}
   ...
   if (k.action == key_action::press || k.action == key_action::repeat) {
      ...
      case key_code::escape: {
         ...
         if (di) {
            ...
            ctx.view.track_drop({pl, ctx.cursor_pos()},
...
element *draggable_element::hit_test(context const &ctx, point p,
                                     bool /*leaf*/) {
...
void draggable_element::begin_tracking(context const &ctx,
      ...
      if (c) {
         ...
         if (is_selected()) {
            std::size_t num_boxes =
                std::min<std::size_t>(count_selected(*c), max_boxes);
            bounds.right += item_offset * num_boxes;
            bounds.bottom += item_offset * num_boxes;

            _drag_image = share(
                floating(bounds, drag_image(link(this->subject()), num_boxes)));
            ...
            if (auto *di = find_parent<drop_inserter_base *>(ctx)) {
               payload pl;
               pl[address_to_string(di)] = {};
               ctx.view.track_drop({pl, track_info.current},
...
void draggable_element::keep_tracking(context const &ctx,
                                      tracker_info &track_info) {
   if (_drag_image) {
      _drag_image->bounds(_drag_image->bounds().move_to(track_info.current.x,
                                                        track_info.current.y));

      if (auto *di = find_parent<drop_inserter_base *>(ctx)) {
         payload pl;
         pl[address_to_string(di)] = {};
         ctx.view.track_drop({pl, track_info.current},
                             cursor_tracking::hovering);
      }
   }
   ctx.view.refresh();
}

void draggable_element::end_tracking(context const &ctx,
                                     tracker_info &track_info) {
   bool maybe_dragged = std::abs(track_info.distance().x) > 10 ||
