...
list::list(composer_ptr composer, bool manage_externally)
    : _composer(composer), _manage_externally(manage_externally),
      _update_request{true}, _move_request{false}, _insert_request{false},
      _erase_request{false} {}

list::list(list const &rhs)
    : _cells{rhs._cells}, _composer{rhs._composer},
      ...
      _layout_id{rhs._layout_id}, _update_request{true}, _move_request{false},
      _insert_request{false}, _erase_request{false}, _request_info{nullptr} {}

list::list(list &&rhs)
    : _cells{std::move(rhs._cells)}, _composer{rhs._composer},
      _manage_externally{rhs._manage_externally},
      _previous_size{rhs._previous_size},
      _previous_window_start{rhs._previous_window_start},
      _previous_window_end{rhs._previous_window_end},
...
std::size_t list::size() const { return _cells.size(); }
...
void list::draw(context const &ctx) {
   ...
   auto it =
       std::lower_bound(_cells.begin(), _cells.end(),
   ...
   if (_manage_externally) {
      ...
      if (new_start != _previous_window_start ||
          new_end != _previous_window_end) {
...
void list::for_each_visible(context const &ctx, for_each_callback f,
                            bool reverse) const {
   ...
   if (reverse) {
      auto it = std::upper_bound(
          _cells.begin(), _cells.end(), main_axis_port_end - main_axis_start,
      ...
      auto it = std::lower_bound(
          _cells.begin(), _cells.end(), main_axis_port_start - main_axis_start,
          [](auto const &cell, double pivot) {
             return (cell.pos + cell.main_axis_size) < pivot;
          });
...
////////////////////////////////////////////////////////////////////////////
float list::get_main_axis_start(const rect &r) const { return r.top; }

float list::get_main_axis_end(const rect &r) const { return r.bottom; }

view_limits
list::make_limits(float main_axis_size,
...
float hlist::get_main_axis_start(const rect &r) const { return r.left; }

float hlist::get_main_axis_end(const rect &r) const { return r.right; }

view_limits
hlist::make_limits(float main_axis_size,
                   cell_composer::limits secondary_axis_limits) const {
   return {{main_axis_size, secondary_axis_limits.min},
