...
struct button_state {
   button_state()
       : value(false), hilite(false), tracking(false), enabled(true) {}
...
// Toggle Button
////////////////////////////////////////////////////////////////////////////
template <typename Base> class basic_toggle_button : public Base {
public:
   using Base::Base;
...
inline bool basic_toggle_button<Base>::click(context const &ctx,
                                             mouse_button btn) {
   if (!this->is_enabled())
      return false;
   ...
   if (btn.down) {
      ...
      if (this->state(!this->value())) // toggle the state
      {
         ...
         _current_state =
             this->value(); // can change in the drag function and so we'll
...
template <typename Base>
inline void basic_toggle_button<Base>::drag(context const &ctx,
                                            mouse_button btn) {
...
// Latching Button
////////////////////////////////////////////////////////////////////////////
template <typename Base> class basic_latching_button : public Base {
public:
   using Base::Base;
...
template <typename Base>
inline bool basic_latching_button<Base>::click(context const &ctx,
                                               mouse_button btn) {
...
template <typename Base>
class basic_choice : public basic_latching_button<Base>,
                     public basic_choice_base {
public:
...
template <typename Base> inline bool basic_choice<Base>::is_selected() const {
   return this->value();
}

template <typename Base> inline void basic_choice<Base>::select(bool state) {
...
inline basic_choice<proxy<Subject, basic_button>> choice(Subject &&subject) {
