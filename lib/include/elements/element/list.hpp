...
public:
   ...
   virtual float main_axis_size(std::size_t index,
                                basic_context const &ctx) const = 0;
};
...
public:
   ...
   static_limits_cell_composer(float min_secondary_axis_size,
                               float main_axis_size, Rest &&...rest);
   ...
   cell_composer::limits
   secondary_axis_limits(basic_context const &ctx) const override;
   float main_axis_size(std::size_t index,
                        basic_context const &ctx) const override;
...
template <typename Base = cell_composer>
using vfixed_derived_limits_cell_composer =
    fixed_derived_limits_cell_composer<Base>;

template <typename Base = cell_composer>
class hfixed_derived_limits_cell_composer
    : public fixed_derived_limits_cell_composer<Base> {
public:
...
public:
   ...
   fixed_length_cell_composer(std::size_t size, Rest &&...rest)
       : Base(std::forward<Rest>(rest)...), _size(size) {}
...
public:
   ...
   function_cell_composer(F &&compose_, Rest &&...rest)
       : Base(std::forward<Rest>(rest)...), _compose(compose_) {}
...
inline auto basic_cell_composer(std::size_t size, F &&compose) {
   ...
   using return_type = vfixed_derived_limits_cell_composer<
       fixed_length_cell_composer<function_cell_composer<ftype>>>;
   return share(return_type{size, std::forward<ftype>(compose)});
}

template <typename F>
[[deprecated("Use basic_vcell_composer instead.")]] inline auto
basic_vertical_cell_composer(std::size_t size, F &&compose) {
...
template <typename F>
[[deprecated("Use basic_hcell_composer instead.")]] inline auto
basic_horizontal_cell_composer(std::size_t size, F &&compose) {
   using ftype = remove_cvref_t<F>;
   using return_type = hfixed_derived_limits_cell_composer<
       fixed_length_cell_composer<function_cell_composer<ftype>>>;
   return share(return_type{size, std::forward<ftype>(compose)});
}
...
////////////////////////////////////////////////////////////////////////////
template <typename F>
inline auto basic_cell_composer(float min_secondary_axis_size,
                                float main_axis_size, std::size_t size,
                                F &&compose) {
   using ftype = remove_cvref_t<F>;
   using return_type = static_limits_cell_composer<
       fixed_length_cell_composer<function_cell_composer<ftype>>>;
   return share(return_type{min_secondary_axis_size, main_axis_size, size,
...
inline auto basic_cell_composer(float min_secondary_axis_size,
   ...
   using return_type = static_limits_cell_composer<
       fixed_length_cell_composer<function_cell_composer<ftype>>>;
   return share(return_type{min_secondary_axis_size, max_secondary_axis_size,
                            main_axis_size, size,
                            std::forward<ftype>(compose)});
}
...
public:
   ...
   void for_each_visible(context const &ctx, for_each_callback f,
...
protected:
   ...
   virtual void set_bounds(rect &r, float main_axis_start,
                           cell_info &info) const;
...
protected:
   view_limits
   make_limits(float main_axis_size,
               cell_composer::limits secondary_axis_limits) const override;
   void set_bounds(rect &r, float main_axis_start,
                   cell_info &info) const override;
...
// Utility to move items in a vector `v` from given `indices` to a new position,
// `pos`.
template <typename T>
inline void move_indices(std::vector<T> &v, std::size_t pos,
                         std::vector<std::size_t> const &indices) {
   // Precondition: The indices should be validly pointing to items in vector
   // `v`.
...
template <typename T>
inline void erase_indices(std::vector<T> &v,
                          std::vector<std::size_t> const &indices) {
   // Precondition: The indices should be validly pointing to items in vector
   // `v`.
   for (auto i = indices.crbegin(); i != indices.crend(); ++i)
...
inline static_limits_cell_composer<Base>::static_limits_cell_composer(
    float min_secondary_axis_size, float main_axis_size, Rest &&...rest)
    : Base(std::forward<Rest>(rest)...), _main_axis_size{main_axis_size},
      _secondary_axis_limits{min_secondary_axis_size, full_extent} {}

template <typename Base>
template <typename... Rest>
inline static_limits_cell_composer<Base>::static_limits_cell_composer(
    float min_secondary_axis_size, float max_secondary_axis_size,
    float main_axis_size, Rest &&...rest)
    : Base(std::forward<Rest>(rest)...), _main_axis_size(main_axis_size),
...
static_limits_cell_composer<Base>::secondary_axis_limits(
    basic_context const & /*ctx*/) const {
   return _secondary_axis_limits;
}

template <typename Base>
inline float static_limits_cell_composer<Base>::main_axis_size(
    std::size_t /*index*/, basic_context const & /*ctx*/) const {
...
template <typename Base>
template <typename... Rest>
inline fixed_derived_limits_cell_composer<
    Base>::fixed_derived_limits_cell_composer(Rest &&...rest)
    : Base(std::forward<Rest>(rest)...), _main_axis_size{-1},
      _secondary_axis_limits{-1, full_extent} {}

template <typename Base>
inline cell_composer::limits
fixed_derived_limits_cell_composer<Base>::secondary_axis_limits(
    basic_context const &ctx) const {
...
template <typename Base>
inline float fixed_derived_limits_cell_composer<Base>::main_axis_size(
    std::size_t /*index*/, basic_context const &ctx) const {
...
template <typename Base>
void fixed_derived_limits_cell_composer<Base>::get_limits(
    basic_context const &ctx) const {
...
template <typename Base>
template <typename... Rest>
inline hfixed_derived_limits_cell_composer<
    Base>::hfixed_derived_limits_cell_composer(Rest &&...rest)
    : fixed_derived_limits_cell_composer<Base>(std::forward<Rest>(rest)...) {}

template <typename Base>
void hfixed_derived_limits_cell_composer<Base>::get_limits(
    basic_context const &ctx) const {
   auto lim = const_cast<base_type *>(this)->compose(0)->limits(ctx);
...
inline void list::set_bounds(context &ctx, float main_axis_pos,
                             cell_info &cell) const {
