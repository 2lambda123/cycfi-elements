...
public:
   ...
   void track_drop(context const &ctx, drop_info const &info,
...
public:
   ...
   using on_drop_function =
       std::function<bool(drop_info const &info, std::size_t ix)>;
   using on_move_function =
       std::function<void(std::size_t pos, indices_type const &indices)>;
   using on_delete_function = std::function<void(indices_type const &indices)>;
   using on_select_function =
       std::function<void(indices_type const &indices, std::size_t latest)>;
   ...
   on_drop_function on_drop = [](drop_info const &, std::size_t) {
      return false;
   };
...
inline bool draggable_element::is_selected() const { return _selected; }

inline void draggable_element::select(bool state) { _selected = state; }
} // namespace elements
} // namespace cycfi
