...
public:
   ...
   template <typename... T> tracker(T &&...args) : Base(args...) {}
...
protected:
   ...
   virtual tracker_info_ptr new_state(context const &ctx, point start,
                                      int modifiers);
   virtual void begin_tracking(context const & /*ctx*/,
                               tracker_info & /*track_info*/) {}
   virtual void keep_tracking(context const & /*ctx*/,
                              tracker_info & /*track_info*/) {}
   virtual void end_tracking(context const & /*ctx*/,
                             tracker_info & /*track_info*/) {}
...
tracker<Base, TrackerInfo>::tracker(tracker const &rhs) : Base(rhs) {}
...
template <typename Base, typename TrackerInfo>
inline bool tracker<Base, TrackerInfo>::click(context const &ctx,
                                              mouse_button btn) {
   Base::click(ctx, btn);
   if (btn.down) {
...
template <typename Base, typename TrackerInfo>
inline void tracker<Base, TrackerInfo>::drag(context const &ctx,
                                             mouse_button btn) {
   Base::drag(ctx, btn);
   if (state) {
...
tracker<Base, TrackerInfo>::new_state(context const & /* ctx */, point start,
                                      int modifiers) {
   return std::make_unique<tracker_info>(start, modifiers);
}

template <typename Base, typename TrackerInfo>
inline void tracker<Base, TrackerInfo>::track_scroll(context const &ctx,
                                                     point /* dir */,
                                                     point /* p */) {
   this->on_tracking(ctx, element::while_tracking);
}
...
template <typename Base, typename TrackerInfo>
inline void tracker<Base, TrackerInfo>::escape_tracking(context const &ctx) {
   if (state) {
      this->on_tracking(ctx, element::end_tracking);
      end_tracking(ctx, *state);
      state.reset();
